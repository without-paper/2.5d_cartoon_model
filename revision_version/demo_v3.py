...
def generate_12_view_orientation_space(model: CartoonModel2_5D):
    ...
    for view in mirrored_views:
        if view not in model.key_views:
            ...
            for stroke_id, stroke in model.strokes.items():
                ...
                if source_view and source_view in stroke.key_views:
    ...
    rotated_views = []
    additional_yaws = [math.pi/4, 3*math.pi/4, -math.pi/4, -3*math.pi/4]
...
def visualize_orientation_space_with_characters(model: CartoonModel2_5D, output_dir: str = "orientation_space_views"):
    ...
    important_views = {
        ...
        (0, math.pi/2): 'Top',
        (0, -math.pi/2): 'Bottom',
        (math.pi/4, math.pi/2): 'Top-Right',
        (3*math.pi/4, math.pi/2): 'Top-Left',
        (-math.pi/4, math.pi/2): 'Top-BackRight',
        (-3*math.pi/4, math.pi/2): 'Top-BackLeft',
        (math.pi/4, -math.pi/2): 'Bottom-Right',
        (3*math.pi/4, -math.pi/2): 'Bottom-Left',
        (-math.pi/4, -math.pi/2): 'Bottom-BackRight',
        (-3*math.pi/4, -math.pi/2): 'Bottom-BackLeft'
    }
    ...
    for i, (yaw, pitch) in enumerate(main_views[:12]):  # Show first 12 main views
        ...
        for stroke_id, points in view_strokes.items():
            if len(points) > 1:
                path = Path(points)
                patch = patches.PathPatch(path, facecolor='none', 
                                        edgecolor=colors[color_idx % len(colors)], 
