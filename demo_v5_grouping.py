...
class CartoonModel2_5D:
    ...
    def interpolate_shape(self, stroke_id: str, target_camera: Camera) -> Optional[np.ndarray]:
        """Interpolate stroke shape and position for target camera view."""
        ...
        if self.delaunay is None or len(stroke.normalized_shapes) < 2:
            ...
            if normalized_shape is None:
                return None
            centroid = stroke.centroids[list(stroke.centroids.keys())[0]]  # Fallback to any centroid
        ...
        if simplex_idx[0] != -1:
            ...
            available_map = {v: s for v, s in zip(triangle_views, [stroke.normalized_shapes.get(v) for v in triangle_views]) if s is not None}
            ...
            if len(available_views) == 0:
                ...
                if normalized_shape is None:
                    return None
                centroid = stroke.centroids[list(stroke.centroids.keys())[0]]
                return normalized_shape + centroid
            ...
            interpolated_centroid = np.average(centroids, axis=0, weights=weights)
            # For face, enforce circularity if desired
            if stroke_id == 'face' and available_views:
                ...
                interpolated_shape = np.column_stack([interpolated_radius * np.cos(angles), interpolated_radius * np.sin(angles)])
            # Combine shape and centroid
            interpolated_points = interpolated_shape + interpolated_centroid
            # Area scaling
            ...
            area_cur = abs(_polygon_area(interpolated_points))
            if area_cur > 1e-8 and area_target > 1e-8:
                scale = math.sqrt(area_target / area_cur)
                c = np.mean(interpolated_points, axis=0, keepdims=True)
                interpolated_points = (interpolated_points - c) * scale + c
