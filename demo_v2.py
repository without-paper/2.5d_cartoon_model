...
class CartoonModel2_5D:
    ...
    def generate_derived_views(self):
        """生成衍生视角（围绕角色中心调整yaw和pitch） with global rotation"""
        new_views = []
        ...
        # 应用全局旋转
        rotation_matrix = self._get_rotation_matrix(group_anchor, 0, 0)  # Identity for initial setup
            ...
            if abs(pitch) == math.pi / 2:
                for new_yaw in np.linspace(-math.pi, math.pi, num=9, endpoint=False):
                    if abs(new_yaw - yaw) > 1e-6 and (new_yaw, pitch) not in self.key_views:
                        ...
                        rotation_matrix = self._get_rotation_matrix(group_anchor, new_yaw - yaw, 0)
